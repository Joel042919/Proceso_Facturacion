...
st.set_page_config(page_title="Sistema de Facturación con PostgreSQL", layout="wide")
...
def main():
    st.title("Sistema de Facturación con PostgreSQL")

    # Inicializar conexión a la base de datos
    if 'db_conn' not in st.session_state:
        st.session_state.db_conn = PostgreSQLConnector()
    ...
    with st.sidebar:
        ...
        username = st.text_input("Usuario", "postgres")
        password = st.text_input("Contraseña", type="password")

        if st.button("Conectar a PostgreSQL"):
